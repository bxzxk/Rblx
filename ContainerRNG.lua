...
local function http_post_json(url, bodyTable)
    ...
    if req then
        local resp = req({ Url = url, Method="POST",
...
-- Parse UI notification to update best item (tolerant to phrasing)
local function scanAndTrackBestItem()
    local pg = localPlayer:FindFirstChild("PlayerGui"); if not pg then return end
    local sg = pg:FindFirstChild("ScreenGui")
    local nf = sg and sg:FindFirstChild("NotificationFrame")
    ...
    -- Matches: "You found X worth $1,234,567" or "Unboxed X worth $1,234"
    local name  = txt:match("found%s+([%w%s%p]-)%s+worth") or txt:match("Unboxed%s+([%w%s%p]-)%s+worth")
    local money = txt:match("%$([%d,]+)")
    local muts  = txt:match("Mutations:%s*(%d+)") or txt:match("Muts:%s*(%d+)")
    local size  = txt:match("Size:%s*([%d%.]+)")  -- e.g. "Size: 2.14"
...
-- Webhook appearance (username + avatar = your Roblox profile)
local function getPlayerAvatarUrl()
    local userId = players.LocalPlayer.UserId
    return ("https://www.roblox.com/headshot-thumbnail/image?userId=%d&width=420&height=420&format=png"):format(userId)
end
...
-- Webhook: best item + money + net profit (+ muts/size)
local function sendBestItemWebhook()
    if not Config.webhookEnabled or (Config.webhookURL or "") == "" then return end
...
-- Background jobs
task.spawn(function() -- A) watch notifications
    while true do
        if Config.trackItems then pcall(scanAndTrackBestItem) end
        task.wait(0.5)
    end
end)

task.spawn(function() -- B) hourly report
    while true do
        for _=1,3600 do task.wait(1) end
        if Config.hourlyEnabled then pcall(sendBestItemWebhook) end
        hourAnchorMoney = getCurrentMoneySafe()
    end
end)

task.spawn(function() -- C) autosave config every 20s
    while true do
        task.wait(20)
        Config.autoBuy       = Config.autoBuy       ~= nil and Config.autoBuy       or false
        Config.autoFarm      = Config.autoFarm      ~= nil and Config.autoFarm      or false
        Config.autoCollect   = Config.autoCollect   ~= nil and Config.autoCollect   or false
        Config.autoSell      = Config.autoSell      ~= nil and Config.autoSell      or false
        saveConfig()
    end
end)

-- =========================
--  Your original toggles/state
-- =========================
local selectedContainer = Config.selectedTier or "Junk"
local autoBuyContainer = Config.autoBuy or false
local autoFarmContainers = Config.autoFarm or false
local autoCollectItems = Config.autoCollect or false
local autoSellItems = Config.autoSell or false
...
local function sell()
    ...
    pcall(function() firesignal(localPlayer.PlayerGui.BackpackGui.Backpack.Hotbar.DropAll.MouseButton1Click) end)
    task.wait(1.5)
    pcall(function()
        StarterGui:SetCore("SendNotification", { Title = "Sold", Text = "Your items were dropped/sold", Duration = 10 })
    end)
...
local autoBuySection = autofarmTab.Section("Auto Buy")
autoBuySection.Toggle("Auto Buy Container", function(on)
    autoBuyContainer = on; Config.autoBuy = on; saveConfig()
end)
local autoContainerSection = autofarmTab.Section("Auto Container")
autoContainerSection.Toggle("Auto Farm Container", function(on)
    autoFarmContainers = on; Config.autoFarm = on; saveConfig()
end)
autoContainerSection.Toggle("Pick Up Items", function(on)
    autoCollectItems = on; Config.autoCollect = on; saveConfig()
end)
autoContainerSection.Toggle("Sell Picked Up Items", function(on)
    autoSellItems = on; Config.autoSell = on; saveConfig()
end)
local autofarmSettingsSection = autofarmTab.Section("Select")
local settingsTab = window.Tab("Settings")
local settingsSection = settingsTab.Section("Settings")
...
-- Webhook UI
local webhookSection = settingsTab.Section("Webhook / Reports")
webhookSection.Toggle("Enable Webhook", function(on) Config.webhookEnabled = on; saveConfig() end)
webhookSection.Toggle("Hourly Embed (Best Item + Money + Profit)", function(on) Config.hourlyEnabled = on; saveConfig() end)
if type(webhookSection.Textbox) == "function" then
    webhookSection.Textbox("Discord Webhook URL", Config.webhookURL or "", function(text)
        Config.webhookURL = text or ""; saveConfig()
    end)
end
webhookSection.Button("Send Best Item Now", function() pcall(sendBestItemWebhook) end)
...
local names = {}; for k in pairs(containerMeta) do table.insert(names, k) end
table.sort(names)
autofarmSettingsSection.Dropdown("Container Name", names, selectedContainer, function(v)
    selectedContainer = v; Config.selectedTier = v; saveConfig()
end)
...
task.spawn(function()
    ...
    repeat
        ...
        pcall(function()
            local sp = localPlayer.PlayerGui.MenusUI:FindFirstChild("StarterPack")
            if sp and sp:FindFirstChild("CloseButton") then firesignal(sp.CloseButton.MouseButton1Click) end
        end)
                    ...
                    if holder and not prompt then
                        if itemCache then
                            for _, item in ipairs(itemCache:GetChildren()) do
                                ...
                                local ok, cf, size = pcall(function() local a,b=model:GetBoundingBox(); return a,b end)
                                if not ok or not cf or not size then continue end
                                local rel, half = cf:PointToObjectSpace(itemPos), size * 0.5
                                local planar = ((itemPos * Vector3.new(1,0,1)) - (contPos * Vector3.new(1,0,1))).Magnitude <= 31
                                if not (planar and math.abs(rel.X) <= half.X and math.abs(rel.Z) <= half.Z) then continue end
                                ...
                                if pos and hrp then
                                    ...
                                    pcall(scanAndTrackBestItem)
                                    local sold = sell(); if sold ~= false then break end
                                end
                            end
                        end
                    else
                ...
                if not (holder and #holder:GetChildren() > 0) then
                    local function openComputer()
                        ...
                        local comp = localPlot.PlotLogic
                            and localPlot.PlotLogic:FindFirstChild("ComputerSystem")
                            and localPlot.PlotLogic.ComputerSystem:FindFirstChild("ComputerProximityHolder")
                        local pos = comp and comp.Position or (comp and comp:GetPivot().Position)
                        if not pos then return false end
                        if tween(Vector3.new(pos.X, pos.Y + 5, pos.Z)) == "no root" then return false end
                        local pp = comp and (comp:FindFirstChildOfClass("ProximityPrompt") or comp:FindFirstChildWhichIsA("ProximityPrompt", true))
                        if not pp then return false end
                        pcall(function() fireproximityprompt(pp) end)
                        return true
                    end
                    local function openContainerShop()
                        local btn; local ok = waitUntil(function() btn = findClickableForLabel("ContainerEx"); return btn ~= nil end, 6)
                        if not ok or not btn then return false end
                        pcall(function() firesignal(btn.MouseButton1Click) end)
                        return true
                    end
                    local function selectContainer(name)
                        local want = name .. " Container"
                        local btn; local ok = waitUntil(function() btn = findClickableForLabel(want, "InfoHolder"); return btn ~= nil end, 6)
                        if not ok or not btn then return false end
                        pcall(function() firesignal(btn.MouseButton1Click) end)
                        return true
                    end
                    local function buyToFill()
                        ...
                        local buyBtn = shop and shop:FindFirstChild("RegularButton", true)
                        if not buyBtn then
                            local fb = findClickableForLabel("Regular"); if fb then buyBtn = fb end
                        end
                        if not buyBtn then return false end
                        for _=1,toFill do pcall(function() firesignal(buyBtn.MouseButton1Click) end); task.wait(0.12) end
                        pcall(function()
                            local closeBtn = shop and shop:FindFirstChild("ButtonHolder") and shop.ButtonHolder:FindFirstChild("CloseButton")
                            if closeBtn then firesignal(closeBtn.MouseButton1Click) end
                        end)
                    ...
                    local function buySelectedContainerWithRetry(name, retries)
                        ...
                        for attempt=1,retries do
                            local hh = localPlot.PlotLogic and localPlot.PlotLogic:FindFirstChild("ContainerHolder")
                            if hh and #hh:GetChildren() > 0 then return true end
                            StarterGui:SetCore("SendNotification", { Title=("Buying (attempt %d/%d)"):format(attempt,retries), Text="Trying to purchase container...", Duration=5 })
                            task.wait(2)
                            if openComputer() and openContainerShop() and selectContainer(name) then
                                task.wait(2)
                    ...
                    local ok = buySelectedContainerWithRetry(selectedContainer, 4)
                    if not ok then
                        StarterGui:SetCore("SendNotification", { Title="Buy failed", Text="Will retry automatically.", Duration=6 })
                    end
                end
            end
        end
    ...
    while true do
        ...
        if char and root then
            if isTweening then
                ...
                for _, v in ipairs(char:GetChildren()) do if v:IsA("BasePart") then v.CanCollide = true end end
            end
        end
        task.wait()
    end
